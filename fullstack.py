...
from secret_api_keys import GOOGLE_API_KEY
...
def initialize_llm():
    """Initialize the LLM with Google API"""
...
def create_outline(llm, topic: str) -> List[Dict]:
    """Generate a blog post outline using Google AI"""
    prompt = PromptTemplate(
    ...
    for line in result.split('\n'):
        ...
        if line.startswith('## '):
            if current_section:
                outline.append(current_section)
            current_section = {'title': line[3:], 'subsections': []}
        elif line.startswith('### '):
            if current_section:
                current_section['subsections'].append(line[4:])
...
def generate_full_content(llm, outline: List[Dict]) -> str:
    """Generate complete blog content using Google AI"""
    sections = []
    progress_bar = st.progress(0)
    status_text = st.empty()
...
def main():
    """Main Streamlit application"""
    st.title("📝 HR Blog Post Generator")
    st.markdown("Generate SEO-optimized HR blog posts using Google AI")
    
    setup_environment()
    
    if not os.environ.get("GOOGLE_API_KEY"):
        st.error("Please set GOOGLE_API_KEY in secrets.toml or environment variables")
        return
    ...
    with st.sidebar:
        ...
        if topic_option == "Choose from trending":
            selected_topic = st.selectbox("Select trending HR topic:", get_trending_topics())
        else:
            selected_topic = st.text_input("Enter your custom topic:", "Remote Work Policies")
    ...
    if generate_button:
        with st.spinner("Initializing Google AI..."):
            llm = initialize_llm()
        
        st.subheader(f"Generating: {selected_topic}")
        
        with st.status("Creating outline..."):
            outline = create_outline(llm, selected_topic)
            st.json([section["title"] for section in outline])
